...
## Set up two by two plot matrix
par(mfcol = c(2, 2))

## Execute plot 2 in first column, first row
library(stringi)
library(lubridate)
...
## Execute plot 3 in first column, second row
...
## Execute new Voltage plot over time in second column, first row
plot(epc$datetime, epc$Voltage, type = "l", xlab = "datetime", ylab = "Voltage")

## Execute new Global reactive power plot over time in second column, second row
